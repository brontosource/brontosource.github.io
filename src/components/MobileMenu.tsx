    ...
    return (
        <div className="lg:hidden">
            ...
            <div
                ...
                className={`fixed top-0 right-0 w-full h-[100dvh] bg-accent-50 bg-opacity-50 z-[9999999] duration-100 ${isOpenMobileMenu
                    ? "opacity-100 pointer-events-auto"
                    : "opacity-0 pointer-events-none delay-300"
                    }`}
            >
                <div
                    className={`w-[80%] sm:w-[300px] justify-between pt-4 pb-4 bg-accent h-full duration-300 flex flex-col float-right	${isOpenMobileMenu ? "translate-x-0 delay-200" : "translate-x-full"
                        }`}
                >
                    ...
                    <ul className="text-center text-off-white h-full">
                        {navLinkList.map((navLink) => (
                            <li key={navLink.label}>
                                <a
                                    onClick={() => setIsOpenMobileMenu(false)}
                                    className="block px-5 py-3.5 text-off-white hover:text-heading"
                                    href={navLink.url}
                                >
                                    {navLink.label}
                                </a>
